...
FlatTubeMark::copy()
{
  FlatTubeMarkRef m = new FlatTubeMark(_name, _gfxMgr, _triStripModel);
...
void
FlatTubeMark::trimEnd(int newEndPt)
{
  if (newEndPt == 0) {
    newEndPt = 1;
  }
...
void
FlatTubeMark::addMarkSpecificSample(BrushStateRef brushState)
{ 
  Vector3 right;
  double width;
  ...
  for (int j=0;j<_nfaces;j++) {
    // store verts
    Vector3 thisVert = center + rvec*cos(a) + 0.25*nvec*sin(a);
    if (n == 1) {
      verts.append(center);
    }
    else {
...
void
FlatTubeMark::updateTriStripModel()
{
  for (int i=0;i<_nfaces;i++) {
    if (_modelID[i] == -1) {
      _modelID[i] = _triStripModel->addModel(_verts[i], _norms[i], _maskTexCoords[i], _patTexCoords[i], _colTexCoords[i], false, getInitialBrushState()->layerIndex, getInitialBrushState()->frameIndex);
    }
    else {
...
void
FlatTubeMark::commitGeometry(ArtworkRef artwork)
{
  if (!_hidden) {
...
void 
FlatTubeMark::pose(Array<PosedModel::Ref>& posedModels, const CoordinateFrame& frame)
{
}

void 
FlatTubeMark::draw(RenderDevice *rd, const CoordinateFrame& frame)
{
  if (_samplePositions.size() >= 2) {
...
void 
FlatTubeMark::setHighlighted(bool highlighted, ArtworkRef artwork)
{
...
void
FlatTubeMark::startEditing(ArtworkRef artwork)
{
  if (_modelID[0] != -1) {
...
void
FlatTubeMark::stopEditing(ArtworkRef artwork)
{
  if (!_highlighted) {
    commitGeometry(artwork);
  }
...
void
FlatTubeMark::transformBy(CoordinateFrame frame)
{
